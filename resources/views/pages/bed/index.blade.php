<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage="Bed"></x-navbars.navs.auth>
        <!-- End Navbar -->
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white mx-3"><strong>Bed<strong>
                            </div>
                        </div>
                        <div class="me-3 mt-3 mb-2 text-end">
                            <a class="btn bg-gradient-dark mb-0" href="{{ route('bed.create') }}"><i
                                    ...
                                    <tbody>
                                        @foreach ($bed as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm">{{ $item->bed_number }}</h6>

                                                    </div>
                                                </td>
                                                <td class="align-middle text-center text-sm">
                                                    <p class="text-xs text-secondary mb-0">{{ $item->availability }}
                                                    </p>
                                                </td>
                                                <td class="align-middle text-center text-sm">
                                                    <p class="text-xs text-secondary mb-0">{{ $item->hostelRoom->room_number }}
                                                    </p>
                                                </td>
                                                <td class="align-middle">
                                                    <a rel="tooltip" class="btn btn-success btn-link"
                                                        href="{{ route('bed.edit', $item->id) }}"
                                                    ...
                                                    <form method="POST"
                                                        action="{{ route('bed.destroy', $item->id) }}"
